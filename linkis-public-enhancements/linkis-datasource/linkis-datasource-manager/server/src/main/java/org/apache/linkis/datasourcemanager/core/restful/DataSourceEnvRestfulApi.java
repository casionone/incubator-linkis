...
@RestController
@RequestMapping(value = "/data-source-manager/env")
public class DataSourceEnvRestfulApi {
    ...
    public Message add(@RequestBody DataSourceEnv dataSourceEnv, HttpServletRequest req)
    ...
    public Message remove(@RequestParam("envId") Long envId, HttpServletRequest request) {
        return RestfulApiHelper.doAndResponse(
                () -> {
                    String userName = SecurityFilter.getLoginUsername(request);
                    if (!RestfulApiHelper.isAdminUser(userName)) {
    ...
    public Message getAllEnvListByDataSourceType(@RequestParam("type") String type) {
        return RestfulApiHelper.doAndResponse(
                () -> {
                    List<DataSourceEnv> envList =
                            dataSourceInfoService.listDataSourceEnvByTypeName(type);
    ...
    public Message getEnvEntityById(@RequestParam("envId") Long envId) {
        return RestfulApiHelper.doAndResponse(
                () -> {
