        ...
        if (needToLoad) {
            ...
            serviceInstance =
                    metaServiceInstances.compute(
                            dsType,
                            new BiFunction<String, MetaServiceInstance, MetaServiceInstance>() {
                                @Override
                                public MetaServiceInstance apply(String key, MetaServiceInstance instance) {
                                    if (null != instance
                                                    ...
                                                    (type, classLoader) -> {
                                                        try {
                                                            return new URLClassLoader(
                                                                    MetaClassLoaderManager.this.getJarsUrlsOfPath(componentLib)
                                                                            .toArray(new URL[0]),
                                                                    parentClassLoader);
                                                        } catch (Exception e) {
                                                            LOG.error(
                                    ...
                                    Class<? extends MetadataService> metaServiceClass =
                                            MetaClassLoaderManager.this.searchForLoadMetaServiceClass(
