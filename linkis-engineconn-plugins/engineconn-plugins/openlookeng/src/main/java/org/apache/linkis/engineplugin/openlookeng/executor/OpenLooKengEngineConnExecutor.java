...
public class OpenLooKengEngineConnExecutor extends ConcurrentComputationExecutor {
    ...
    private OkHttpClient okHttpClient = new OkHttpClient.Builder().socketFactory(new SocketChannelSocketFactory())
    ...
    private Cache<String, ClientSession> clientSessionCache = CacheBuilder
            .newBuilder()
            .expireAfterAccess((Long) EngineConnConf.ENGINE_TASK_EXPIRE_TIME().getValue(), TimeUnit.MILLISECONDS)
    ...
    public ExecuteResponse execute(EngineConnTask engineConnTask) {
        ...
        clientSessionCache.put(engineConnTask.getTaskId(), getClientSession(user, engineConnTask.getProperties(), OpenLooKengEngineConfCache.getConfMap(userCreatorLabel, engineTypeLabel)));
    ...
    public ExecuteResponse executeLine(EngineExecutionContext engineExecutorContext, String code) {
        ...
        StatementClient statement = StatementClientFactory.newStatementClient(okHttpClient, clientSession, code);
    ...
    public NodeResource getCurrentNodeResource() {
        ...
        if (properties.containsKey(EngineConnPluginConf.JAVA_ENGINE_REQUEST_MEMORY().key())) {
            String settingClientMemory = properties.get(EngineConnPluginConf.JAVA_ENGINE_REQUEST_MEMORY().key());
        ...
        LoadResource usedResource = new LoadResource(EngineConnPluginConf.JAVA_ENGINE_REQUEST_MEMORY().getValue(properties).toLong(),
    ...
    private ClientSession getClientSession(String user, Map<String, Object> taskParams, Map<String, String> cacheMap) {
        ...
        for (Map.Entry<String, String> keyValue : configMap.entrySet()) {
            if (keyValue.getKey().startsWith("presto.session.")) {
                properties.put(keyValue.getKey().substring("presto.session.".length()), keyValue.getValue());
            }
        }
        ...
        io.airlift.units.Duration clientRequestTimeout = new io.airlift.units.Duration(0, TimeUnit.MILLISECONDS);

        ClientSession session = new ClientSession(httpUri, user, source, traceToken, clientTags, clientInfo, catalog, schema, "", timeZonId, locale,
    ...
    private void queryOutput(String taskId, EngineExecutionContext engineExecutorContext, StatementClient statement) throws IOException {
        int columnCount = 0;
        int rows = 0;
        ResultSetWriter<? extends MetaData, ? extends Record> resultSetWriter = engineExecutorContext.createResultSetWriter(ResultSetFactory$.MODULE$.TABLE_TYPE());
        try {
            ...
            org.apache.linkis.storage.domain.Column[] columns = results.getColumns().stream()
                    .map(column -> new org.apache.linkis.storage.domain.Column(column.getName(), DataType.toDataType(column.getType()), ""))
    ...
    private ErrorExecuteResponse verifyServerError(String taskId, EngineExecutionContext engineExecutorContext, StatementClient statement) throws OpenLooKengClientException, OpenLooKengStateInvalidException {
        ...
        if (statement.isFinished()) {
            ...
            throw new OpenLooKengStateInvalidException(60002, "openlookeng status error. Statement is not finished.");
        }
        return null;
    }
